...
fn solve_in_parallel(puzzle: &str) -> Vec<String> {
    ...
    let possible_values_map = Arc::new(RwLock::new(HashMap::<usize, Vec<u8>>::new()));
    let tasks = GridTask::generate_tasks(Arc::clone(&arc_puzzle), Arc::clone(&possible_values_map));
    ...
    while arc_tasks.iter().any(|t| !t.lock().unwrap().done()) {
        ...
        if arc_tasks.iter().any(|t| t.lock().unwrap().possible_values_count() == 0) {
            return vec![];
        }
    ...
    recursion_base
        ...
        .for_each(|&v| {
            ...
            res.extend(solve_in_parallel(&recursion_puzzle));
        });
...
fn solve_sequentially(puzzle: &str) -> Vec<String> {
    ...
    while tasks.iter().any(|t| !t.done()) {
        ...
        if tasks.iter().any(|t| t.possible_values_count() == 0) {
            return vec![];
        }
